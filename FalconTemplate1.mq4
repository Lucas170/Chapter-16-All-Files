...
int start()
  {
   ...
   if(CountPosOrders(MagicNumber, OP_BUY)>=1 || CountPosOrders(MagicNumber, OP_SELL)>=1){
      if(CountPosOrders(MagicNumber, OP_BUY)>=1 && ExitSignal(CrossTriggered)==1){ // Close Long Positions
         ClosePosition(OP_BUY, OnJournaling, MagicNumber, Slippage, P); 
      }
      if(CountPosOrders(MagicNumber, OP_SELL)>=1 && ExitSignal(CrossTriggered)==2){ // Close Short Positions
         ClosePosition(OP_SELL, OnJournaling, MagicNumber, Slippage, P);
      }
   }   
...
11) VolBasedStopLoss
12) VolBasedTakeProfit
13) Crossed
14) HandleTradingEnvironment
15) GetErrorDescription
...
double GetLot(bool IsSizingOnTrigger,double FixedLots, double RiskPerTrade, int YenAdjustment, double STOP, int K) {
   ...
   if (IsSizingOnTrigger == true) {
      output = RiskPerTrade * 0.01 * AccountBalance() / (MarketInfo(Symbol(),MODE_LOTSIZE) * STOP * K * Point); // Sizing Algo based on account size
...
// This function submits new orders
   ...
   double stoploss=0;
   double takeprofit=0;
   if(SL!=0 && SL<=MathCeil(MarketInfo(Symbol(),MODE_STOPLEVEL)/K))SL=MathCeil(MarketInfo(Symbol(),MODE_STOPLEVEL)/K);
   if(TP!=0 && TP<=MathCeil(MarketInfo(Symbol(),MODE_STOPLEVEL)/K))TP=MathCeil(MarketInfo(Symbol(),MODE_STOPLEVEL)/K);
   int Ticket=-1;
   double price=0;
   if(!ECN)
     {
...
//| CLOSE/DELETE ORDERS 
//+------------------------------------------------------------------+
bool ClosePosition(int TYPE, bool Journaling, int Magic, int Slip , int K){
// Type: Fixed Template 
// Do not edit unless you know what you're doing

// This function closes current positions
